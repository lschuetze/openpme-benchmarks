...
typedef grid_dist_id<3,float,aggregate<float,float,float,float> > GridType;

void init(GridType & grid, Box<3,float> & domain)
{
	//! \cond [create points] \endcond

	typedef typename GetAddBlockType<GridType>::type InsertBlockT;
...
int main(int argc, char* argv[])
{
	...
	grid_dist_id<3, float, aggregate<float,float,float,float>> grid(sz,domain,g,bc);
